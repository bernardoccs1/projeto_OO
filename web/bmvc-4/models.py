...
class Notificacao:
    ...
    def __init__(self, tipo, mensagem, usuario_id=None):
        self.id = len(Notificacao.notificacoes) + 1
        self.tipo = tipo
    ...
    @classmethod
    def criar(cls, tipo, mensagem, usuario_id=None):
        notif = cls(tipo, mensagem, usuario_id)
        cls.notificacoes.append(notif)
        if len(cls.notificacoes) > 50:
            cls.notificacoes.pop(0)
        return notif
    
    @classmethod
    def obter_ultimas(cls, usuario_id=None, limite=10):
        if usuario_id:
            notifs = [n for n in cls.notificacoes if n.usuario_id == usuario_id or n.usuario_id is None]
        else:
            notifs = cls.notificacoes
        return notifs[-limite:]
